        ...
        if has_name("_NewEnum"):
            ...
            class _(object):
                def __iter__(self):
                    ...
                    # IEnumVARIANT
                    from winapp.comtypes.automation import IEnumVARIANT
...
def CoGetClassObject(clsid, clsctx=None, pServerInfo=None, interface=None):
    if clsctx is None:
        clsctx = CLSCTX_SERVER
    if interface is None:
        import winapp.comtypes.server
        interface = winapp.comtypes.server.IClassFactory
    p = POINTER(interface)()
